...
def test_validate_existing_file(tmp_path):
    f = tmp_path / "foo.txt"
    f.write_text("hello")
    resolved = PathValidator.validate_path(str(f), must_exist=True, must_be_file=True)
...
def test_base_directory_restriction(tmp_path):
    ...
    resolved = PathValidator.validate_path(inside, base_directory=base)
